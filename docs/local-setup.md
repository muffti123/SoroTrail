...
`docker-compose.yml` provides three environments as [Compose profiles](https://docs.docker.com/compose/profiles/);
start exactly one. `dev` (the default you want here) brings up Postgres and
the indexer together against the public Stellar testnet RPC:

```sh
docker compose --profile dev up --build        # foreground
# or
docker compose --profile dev up -d             # background
```

The API is on `http://localhost:8080`. To ingest only specific contracts,
set `WATCHED_CONTRACTS` (see `docker-compose.yml`).

Other profiles, documented in the [README](../README.md#docker-compose):

```sh
docker compose --profile test up -d   # disposable DB on :5433 for make test-db
docker compose --profile prod up -d   # production-flavored stack (no DB on host)
```
...
| Build & push image | `docker compose --profile dev build` (see `Dockerfile`, `RELEASING.md`) |
...
- **Integration tests skip** — Docker isn't available or `TEST_DATABASE_URL`
  ...
  `docker compose --profile test up -d` and export
